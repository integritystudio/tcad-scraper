...
export interface SearchClient<T> {
   ...
   * page is 0-based.
   */
  search(query: string, page: number, pageSize: number): Promise<SearchResponse<T>>;
...
export async function* exploreByAdaptivePrefixes<T>(
  ...
  const seeds = (opts.seeds && opts.seeds.length > 0)
    ? opts.seeds
  ...
  async function processPrefix(prefix: string): Promise<T[] | { expand: string }> {
    ...
    // Decide paging plan
    const shouldPage =
      typeof total === "number" ? total > first.items.length : first.items.length === pageSize;
