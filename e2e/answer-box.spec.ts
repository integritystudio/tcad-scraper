...
test.describe("AnswerBox for quantitative queries", () => {
  ...
  test("shows statistics grid with average and total values", async ({
    ...
    await expect(page.getByText("Average Value")).toBeVisible({ timeout: 10_000 });
  ...
  test("does not show answer box for non-quantitative queries", async ({
    ...
    await expect(page.locator(".results-grid h3").first()).toBeVisible({ timeout: 10_000 });
    await expect(page.getByRole("status", { name: /AI-generated answer/i })).not.toBeVisible();
  });
});
