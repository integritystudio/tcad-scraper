...
export type NaturalLanguageSearchBody = z.infer<
...
export function transformPropertyToAPI(
	...
	const [streetAddress, ..._addressParts] = dbProperty.propertyAddress
		.split(",")
		.map((s) => s.trim());
	...
	return {
		...
		owner: {
			"@type":
				dbProperty.name.includes("LLC") || dbProperty.name.includes("INC")
					? "Organization"
					: "Person",
...
function getSchemaType(
	propType: string,
): "RealEstateListing" | "Place" | "Residence" {
