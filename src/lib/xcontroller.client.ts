...
export class DataController {
	 ...
	 */
	loadData<T = unknown>(id: string): T | null {
		// Check cache first
		...
		try {
			...
			if (!scriptElement) {
				if (this.debug) {
					console.error(
						`[DataController] Script tag with id="${id}" not found`,
					);
				}
				return null;
			}
			...
			if (data === null) {
				if (this.debug) {
					console.error(
						`[DataController] Data in script tag id="${id}" is null`,
					);
				}
				return null;
			}
			...
			if (this.debug) {
				console.error(
					`[DataController] Error loading data from id="${id}":`,
					error,
				);
			}
			return null;
	 ...
	 */
	async loadDataWithFallback<T = unknown>(
		id: string,
		fallbackUrl: string,
	): Promise<T | null> {
