...
app.get("/stats", async (c) => {
  ...
  const [totalLogs, successfulLogs, totalCost, usageByDay, usageByModel, recentLogs] =
    await Promise.all([
      ...
      prisma.apiUsageLog.findMany({
        ...
        take: 10,
        select: {
          id: true, queryText: true, queryCost: true, inputTokens: true,
          outputTokens: true, model: true, environment: true, success: true,
          errorMessage: true, responseTime: true, timestamp: true,
        },
      }),
    ]);

  const successRate = totalLogs > 0 ? (Number(successfulLogs) / totalLogs) * PERCENT_MULTIPLIER : 0;
  ...
  return c.json({
    ...
    byModel: usageByModel.map((model) => ({
      ...
      avgResponseTime: model._avg.responseTime ? `${Math.round(model._avg.responseTime)}ms` : "N/A",
    })),
...
app.get("/logs", async (c) => {
  const prisma = c.get("prisma");
  const limitNum = Math.min(parseInt(c.req.query("limit") || "50", 10) || 50, 1000);
  ...
  const [logs, total] = await Promise.all([
    prisma.apiUsageLog.findMany({ where, orderBy: { timestamp: "desc" }, take: limitNum, skip: offsetNum }),
    prisma.apiUsageLog.count({ where }),
  ]);
...
app.get("/alerts", async (c) => {
  ...
  if (todayCostNum >= DAILY_COST_CRITICAL) {
    alerts.push({ level: "critical", message: `Daily API cost (${todayCostNum.toFixed(2)}) exceeded critical threshold ($${DAILY_COST_CRITICAL})` });
  } else if (todayCostNum >= DAILY_COST_WARNING) {
    alerts.push({ level: "warning", message: `Daily API cost (${todayCostNum.toFixed(2)}) exceeded warning threshold ($${DAILY_COST_WARNING})` });
  }

  if (monthCostNum >= MONTHLY_COST_CRITICAL) {
    alerts.push({ level: "critical", message: `Monthly API cost (${monthCostNum.toFixed(2)}) exceeded critical threshold ($${MONTHLY_COST_CRITICAL})` });
  } else if (monthCostNum >= MONTHLY_COST_WARNING) {
    alerts.push({ level: "warning", message: `Monthly API cost (${monthCostNum.toFixed(2)}) exceeded warning threshold ($${MONTHLY_COST_WARNING})` });
  }

  if (recentFailures >= FAILURE_THRESHOLD) {
    alerts.push({ level: "critical", message: `High failure rate: ${recentFailures} failures in the last 24 hours` });
  }

  return c.json({
    alerts,
    costs: { today: `$${todayCostNum.toFixed(COST_DECIMAL_PLACES)}`, month: `$${monthCostNum.toFixed(COST_DECIMAL_PLACES)}` },
