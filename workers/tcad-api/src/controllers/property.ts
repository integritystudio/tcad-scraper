...
import type { AnswerStatistics, AnswerType, PropertyFilters } from "../types/property.types";
import {
  historyQuerySchema,
...
app.post("/scrape", apiKeyAuth, validateBody(scrapeRequestSchema), async (c) => {
  ...
  return c.json({ message: "Scrape job queued successfully", searchTerm }, 202);
});
...
app.post("/search", validateBody(naturalLanguageSearchSchema), async (c) => {
  const prisma = c.get("prisma");
  const { query, limit = DEFAULT_QUERY_LIMIT, offset = 0 } = c.get("validatedBody") as {
    query: string;
    limit?: number;
    offset?: number;
  };
  ...
  try {
    ...
    return c.json(
      { error: "AI service unavailable", message: "Unable to process natural language query." },
      503,
    );
  }
  ...
  try {
    ...
    if (answer) {
      ...
      statistics = {
        ...
        priceRange:
          aggregates._min.appraisedValue !== null && aggregates._max.appraisedValue !== null
...
app.get("/history", validateQuery(historyQuerySchema), async (c) => {
  const prisma = c.get("prisma");
  const { limit = 20, offset = 0 } = c.get("validatedQuery") as { limit?: number; offset?: number };

  const [jobs, total] = await Promise.all([
    prisma.scrapeJob.findMany({ orderBy: { startedAt: "desc" }, skip: offset, take: limit }),
...
app.post("/monitor", apiKeyAuth, validateBody(monitorRequestSchema), async (c) => {
  const prisma = c.get("prisma");
