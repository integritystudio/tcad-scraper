...
describe("enqueueBatch", () => {
	...
	it("calls injected logger.error when a term fails, not console.error", async () => {
		...
		const count = await enqueueBatch(
			["fail-term", "ok-term"],
			"user-1",
			mockLogger,
		);
...
describe("waitForQueueDrain", () => {
	...
	it("polls until both active and waiting counts reach 0", async () => {
		...
		mockGetWaitingCount.mockResolvedValueOnce(5).mockResolvedValueOnce(0);
		mockGetActiveCount.mockResolvedValueOnce(2).mockResolvedValueOnce(0);
