...
async function main() {
	...
	for (const base of analytics) {
		for (let c = 97; c <= 122; c++) {
			...
			if (searched.has(lower) || blacklistSet.has(lower) || seen.has(lower))
				continue;
	...
	while (idx < expansions.length) {
		...
		console.log(
			`Enqueued batch ${Math.ceil(idx / BATCH_SIZE)}: ${batch.join(", ")}`,
		);
		...
		if (newProps === 0) {
			consecutiveZeros++;
			console.log(
				`  -> 0 new properties (${consecutiveZeros}/${MAX_CONSECUTIVE_ZEROS} consecutive zeros)`,
			);
		} else {
			consecutiveZeros = 0;
			console.log(
				`  -> +${newProps} new properties (total DB: ${afterCount.toLocaleString()})`,
			);
		}
