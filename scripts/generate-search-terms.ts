...
if (Number.isNaN(TARGET_TERM_COUNT) || TARGET_TERM_COUNT <= 0) {
...
async function getDenseTermExpansions(
	searched: Set<string>,
): Promise<string[]> {
...
function* generateUnexploredPrefixes(searched: Set<string>): Generator<string> {
	...
	const shuffled1 = [...HIGH_FREQ_FIRST].sort(
		(a, b) => simpleHash(`1${a}`) - simpleHash(`1${b}`),
	);
	const shuffled2 = [...ALPHABET].sort(
		(a, b) => simpleHash(`2${a}`) - simpleHash(`2${b}`),
	);
	const shuffled3 = [...ALPHABET].sort(
		(a, b) => simpleHash(`3${a}`) - simpleHash(`3${b}`),
	);
	const shuffled4 = [...ALPHABET].sort(
		(a, b) => simpleHash(`4${a}`) - simpleHash(`4${b}`),
	);
...
async function main() {
	...
	console.log(
		`  Already searched: ${searched.size} terms (${successful.size} successful)`,
	);
	...
	console.log(
		`  Already searched: ${searched.size} (${successful.size} successful)`,
	);
