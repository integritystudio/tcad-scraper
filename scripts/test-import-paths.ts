...
const EXTENSIONS = ['.ts', '.tsx', '.js', '.jsx', '.json', '.css', '.module.css'];
 ...
 */
function extractImports(filePath: string, content: string): Array<{ path: string; line: number }> {
 ...
 */
function resolveImport(fromFile: string, importPath: string): { exists: boolean; resolvedPath: string; reason?: string } {
  ...
  if (importPath.startsWith('.')) {
    ...
    if (fs.existsSync(basePath)) {
      ...
      if (stat.isDirectory()) {
        ...
        return { exists: false, resolvedPath: basePath, reason: 'Directory has no index file' };
      }
  ...
  if (importPath.startsWith('/')) {
    ...
    return { exists: false, resolvedPath: absolutePath, reason: 'File not found' };
  }

  return { exists: true, resolvedPath: importPath, reason: 'Assumed external package' };
}
