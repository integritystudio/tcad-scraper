...
program
	.name("queue-manager")
	.description(
		"Manage scraper job queue - consolidates queue management utilities",
	)
	.version("1.0.0");
...
program
	...
	.option(
		...
		"Set priority level (1-10, lower is higher priority)",
		"10",
	)
	.action(async (file: string, options: QueueManagerOptions) => {
		...
		const priorityLevel = options.priority
			? 1
			: parseInt(options.priorityLevel || "10", 10);

		logger.info(`\n🚀 Adding ${terms.length} jobs to queue...`);
		logger.info(
			`   Priority: ${priorityLevel} ${options.priority ? "(highest)" : ""}`,
		);
	...
	.action(async (options: QueueManagerOptions) => {
		...
		if (waiting + delayed === 0 && !options.force) {
			...
			if (active > 0) {
				...
				logger.info(
					"   They will finish processing. Use --force to attempt to fail them.",
				);
			}
			await cleanup();
			return;
		...
		if (options.force && active > 0) {
			...
			for (const job of activeJobs) {
				try {
					...
					if (removed % 10 === 0) {
						process.stdout.write(
							`\r   Progress: ${removed}/${waiting + delayed + active} jobs`,
						);
					}
				} catch (error: unknown) {
					failed++;
...
program
	...
	.action(async (options: QueueManagerOptions) => {
		...
		if (options.zeroResults) {
			...
			logger.info(
				`   Found ${emptyTerms.size} terms that returned zero results`,
			);
		...
		if (options.detailed) {
			...
			if (active > 0) {
				...
				activeJobs.forEach((job, idx) => {
					logger.info(
						`   ${idx + 1}. "${job.data.searchTerm}" (ID: ${job.id})`,
					);
				});
			}
			...
			if (waiting > 0) {
				...
				waitingJobs.forEach((job, idx) => {
					logger.info(
						`   ${idx + 1}. "${job.data.searchTerm}" (Priority: ${job.opts.priority || 10})`,
					);
				});
			}
			...
			if (failed > 0) {
				...
				failedJobs.forEach((job, idx) => {
					logger.info(
						`   ${idx + 1}. "${job.data.searchTerm}" - ${job.failedReason || "Unknown error"}`,
					);
				});
			}
		}
