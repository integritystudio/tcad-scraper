...
program
	...
	.action(async () => {
		...
		jobStats.forEach((stat) => {
			...
			logger.info(
				`   ${stat.status}: ${stat._count._all} jobs (${(stat._sum.resultCount || 0).toLocaleString()} properties)`,
			);
		});
		...
		logger.info(
			`   Total Properties Scraped: ${totalScraped.toLocaleString()}`,
		);
		...
		logger.info("\n📈 Scrape Performance:");
		logger.info(
			`   Average properties per scrape: ${avgStats._avg.resultCount?.toFixed(0) || 0}`,
		);
		logger.info(
			`   Max properties in single scrape: ${avgStats._max.resultCount || 0}`,
		);
		logger.info(
		...
		recentJobs.forEach((job, idx) => {
			const time = job.completedAt
				...
				: "N/A";
			logger.info(
				`   ${idx + 1}. "${job.searchTerm}": ${job.resultCount} properties (${time})`,
			);
		});
	...
	.action(async (options: StatsOptions) => {
		...
		if (options.byCity) {
			const propertiesByCity = await prisma.property.groupBy({
				...
				take: parseInt(options.top || "0", 10),
			});
		...
		logger.info("\n💰 Property Values:");
		logger.info(
			`   Average Appraised: $${(valueStats._avg.appraisedValue || 0).toLocaleString()}`,
		);
		logger.info(
			`   Max Appraised: $${(valueStats._max.appraisedValue || 0).toLocaleString()}`,
		);
		logger.info(
			`   Min Appraised: $${(valueStats._min.appraisedValue || 0).toLocaleString()}`,
		);
		logger.info(
			`   Average Assessed: $${(valueStats._avg.assessedValue || 0).toLocaleString()}`,
		);
...
program
	...
	.action(async (options: StatsOptions) => {
		...
		const totalProperties = recentJobs.reduce(
		...
		logger.info(
			`   Avg properties per job: ${(totalProperties / recentJobs.length || 0).toFixed(1)}`,
		);
...
program
	...
	.action(async (options: StatsOptions) => {
		...
		topTerms.forEach((term, idx) => {
			logger.info(
				`   ${idx + 1}. "${term.searchTerm}": ${(term.resultCount || 0).toLocaleString()} properties`,
			);
		});
		...
		logger.info(
			`📊 Average results per successful term: ${(avgResults._avg.resultCount || 0).toFixed(1)}`,
		);
...
program
	...
	.action(async (options: StatsOptions) => {
		...
		const priorityJobs = await prisma.scrapeJob.findMany({
			...
			take: parseInt(options.recent || "0", 10),
		});
		...
		logger.info(
			`📈 Average per priority job: ${(totalResults / priorityJobs.length).toFixed(1)}`,
		);
			...
			const status =
				job.status === "completed"
					...
					: job.status === "failed"
						...
						: "⏳";
			const results =
				job.status === "completed" ? ` (${job.resultCount} props)` : "";
