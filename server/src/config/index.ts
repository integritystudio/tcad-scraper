 ...
 */
export function parseTcadYear(key: string, defaultValue: number): number {
	...
	claude: {
		apiKey:
			...
			(() => {
				...
				try {
					const key = execSync(
						"doppler secrets get ANTHROPIC_API_KEY -p personal-info -c dev --plain",
						{
							encoding: "utf-8",
					...
					if (key?.startsWith("sk-ant-")) {
						logger.info(
							"Using ANTHROPIC_API_KEY from Doppler personal-info/dev",
						);
						return key;
					}
				} catch (_error) {
...
export function logConfigSummary(): void {
	...
	logger.info(
		`Auth: ${config.auth.apiKey ? "API Key configured" : "No API Key"}, ${config.auth.jwt.secret ? "JWT configured" : "No JWT"}`,
	);
