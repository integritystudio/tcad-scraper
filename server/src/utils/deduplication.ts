...
export async function removeDuplicatesFromQueue(
	...
	const termMap = new Map<
		string,
		Array<{ job: ScraperJob; priority: number; state: string }>
	>();
	...
	const duplicateTerms = Array.from(termMap.entries()).filter(
		([_, jobs]) => jobs.length > 1,
	);

	// Find jobs that were already completed
	const alreadyCompletedTerms = Array.from(termMap.entries()).filter(
		([term, _]) => completedTermSet.has(term),
	);
	...
	if (verbose) {
		...
		logger.info(
			`   ❌ Terms with duplicate pending jobs: ${duplicateTerms.length}`,
		);
		logger.info(
			`   ✅ Terms already completed: ${alreadyCompletedTerms.length}`,
		);
	}
	...
	if (verbose) {
		if (duplicateTerms.length > 0) {
			...
			duplicateTerms.slice(0, displayCount).forEach(([term, jobs]) => {
				logger.info(
					`   "${term}": ${jobs.length} copies (keeping 1, removing ${jobs.length - 1})`,
				);
			});
		...
		if (alreadyCompletedTerms.length > 0) {
			...
			if (alreadyCompletedTerms.length > displayCount) {
				logger.info(
					`   ... and ${alreadyCompletedTerms.length - displayCount} more`,
				);
			}
			logger.info("");
		}
