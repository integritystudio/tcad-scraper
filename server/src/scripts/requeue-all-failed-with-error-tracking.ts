...
async function saveErrorReport(
	errorMap: Map<string, ErrorStats>,
): Promise<string> {
	const reportData = {
		timestamp: new Date().toISOString(),
		totalFailedJobs: Array.from(errorMap.values()).reduce(
			(sum, stat) => sum + stat.count,
			0,
		),
		errorBreakdown: Array.from(errorMap.values()).sort(
			(a, b) => b.count - a.count,
		),
	};
...
async function main() {
	...
	try {
		...
		const totalFailed = Array.from(errorMap.values()).reduce(
			(sum, stat) => sum + stat.count,
			0,
		);
		...
		logger.info(
			`   Found ${termsToRequeue.length} unique search terms to re-queue`,
		);
		...
		for (let i = 0; i < termsToRequeue.length; i += BATCH_SIZE) {
			...
			logger.info(
				`   ✅ Batch ${batchNum}/${totalBatches} enqueued (${enqueued} total)`,
			);
