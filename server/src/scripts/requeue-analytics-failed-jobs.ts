...
async function main() {
	...
	try {
		...
		logger.info(
			`   Found ${termsToRequeue.length} unique search terms with analytics errors`,
		);
		...
		const [finalWaiting, finalActive, finalDelayed, finalFailed] =
			await Promise.all([
		...
		logger.info(
			`   - Total in queue: ${finalWaiting + finalActive + finalDelayed}`,
		);
		...
		logger.info(
			"   Jobs will no longer fail if analytics updates have issues.\n",
		);
