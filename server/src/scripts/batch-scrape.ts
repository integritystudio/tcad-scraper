...
const _STREET_PREFIXES = ["North", "South", "East", "West", "N", "S", "E", "W"];
...
class BatchScraper {
	...
	async queueJobs(): Promise<void> {
		...
		logger.info(
			`Starting batch scrape with ${searchTerms.length} search terms using strategy: ${this.config.searchStrategy}`,
		);
		logger.info(
			`Batch size: ${this.config.batchSize}, Delay: ${this.config.delayBetweenBatches}ms`,
		);
...
const delay = parseInt(args[2], 10) || 5000;
