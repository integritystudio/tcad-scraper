...
class SearchPatternGenerator {
	...
	async loadUsedTermsFromDatabase(forceRefresh = false): Promise<void> {
		...
		if (
			!forceRefresh &&
			this.dbTermsLoaded &&
			now - this.lastDbRefresh < this.DB_REFRESH_INTERVAL
		) {
		...
		logger.info(
			isRefresh
				? "🔄 Refreshing search terms from database..."
				: "📚 Loading previously used search terms from database...",
		);
		...
		try {
			...
			if (isRefresh) {
				logger.info(
					`✅ Refreshed: ${currentCount.toLocaleString()} total terms (${newTermsFound} new since last check)`,
				);
			} else {
				logger.info(
					`✅ Loaded ${currentCount.toLocaleString()} previously used search terms`,
				);
				logger.info(
					`   Will avoid duplicates like: "Estate", "Family", "Trust", etc.`,
				);
			}
	...
	private generatePropertyType(): string {
		return this.propertyTypes[
			Math.floor(Math.random() * this.propertyTypes.length)
		];
	}
	...
	private generateBusinessName(): string {
		...
		const suffix =
			this.businessSuffixes[
				Math.floor(Math.random() * this.businessSuffixes.length)
			];
		return `${name} ${suffix}`;
	}
	...
	private generateNeighborhood(): string {
		return this.neighborhoods[
			Math.floor(Math.random() * this.neighborhoods.length)
		];
	}
	...
	private async optimizeStrategy(): Promise<string[]> {
		...
		try {
			...
			logger.info(
				`   Avg results per search: ${stats.avgResultsPerSearch.toFixed(1)}`,
			);
			...
			if (stats.topPerformers.length > 0) {
				...
				stats.topPerformers.slice(0, 5).forEach((term, i) => {
					logger.info(
						`   ${i + 1}. "${term.searchTerm}" - ${term.avgResultsPerSearch.toFixed(0)} avg results, ${(term.successRate * 100).toFixed(0)}% success`,
					);
				});
			}
			...
			logger.info(
				`✨ Generated ${optimizedTerms.length} high-efficiency terms to prioritize`,
			);
			logger.info(
				`💡 Suggested ${suggestedTerms.length} new terms based on successful patterns\n`,
			);
	...
	async getNextBatch(batchSize: number): Promise<string[]> {
		...
		if (stats.tooCommonTerms > 0) {
			logger.info(
				`   ⏱️  Skipped ${stats.tooCommonTerms} too-common terms (cause API timeouts)`,
			);
		}
		if (stats.businessSupersets > 0) {
			logger.info(
				`   🏢 Skipped ${stats.businessSupersets} business entity supersets`,
			);
		}
...
class ContinuousBatchScraper {
	...
	async run() {
		...
		logger.info(
			`Starting property count: ${this.stats.startingPropertyCount.toLocaleString()}`,
		);
		logger.info(`Target: ${TARGET_PROPERTIES.toLocaleString()}`);
		logger.info(
			`Remaining: ${(TARGET_PROPERTIES - this.stats.startingPropertyCount).toLocaleString()}\n`,
		);
	...
	private async queueBatch() {
		...
		logger.info(
			`✓ Queued ${searchTerms.length} jobs (Total: ${this.stats.totalQueued})`,
		);
	}
...
└─────────────────────────────────────────────────────────┘
        `);
				...
				if (rate > 0) {
					...
					logger.info(
						`⏱️  Estimated time to target: ${hoursRemaining.toFixed(1)} hours`,
					);
				}
			} catch (error) {
