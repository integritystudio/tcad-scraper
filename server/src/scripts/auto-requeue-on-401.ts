...
async function check401Errors() {
  ...
  try {
    ...
    if (recentErrors > 0) {
      logger.warn(`⚠️  Detected ${recentErrors} new 401 errors since last check!`);
      ...
      const termsToRequeue = Array.from(new Set(failedJobs.map(j => j.searchTerm)));
      logger.info(`📋 Found ${termsToRequeue.length} unique terms with 401 errors`);
      ...
      logger.info(`🚀 Re-enqueueing ${termsToRequeue.length} jobs in batches of ${BATCH_SIZE}...`);
      ...
      logger.info(`✅ Re-enqueued all ${termsToRequeue.length} jobs successfully`);

      // Reset consecutive error count after successful requeue
      consecutiveErrorCount = 0;

    } else {
      logger.info(`✅ No new 401 errors detected (checked ${recentErrors} jobs)`);
      consecutiveErrorCount = 0;
    }
...
async function main() {
  ...
  logger.info(`   Token refresh interval: ${TOKEN_REFRESH_INTERVAL_MS / 1000}s`);
