...
async function testApi() {
	...
	const officeLookupResponse = await fetch(
		"https://prod-container.trueprodigyapi.com/trueprodigy/officelookup/travis.prodigycad.com",
	);
	...
	const searchResponse = await fetch(
		"https://prod-container.trueprodigyapi.com/public/property/searchfulltext?page=1&pageSize=5",
		{
			method: "POST",
			headers: {
			...
			body: JSON.stringify({
				pYear: { operator: "=", value: `${config.scraper.tcadYear}` },
	...
	logger.info(
		`✓ Total properties found: ${searchData.totalProperty?.propertyCount || 0}\n`,
	);
	...
	if (searchData.results && searchData.results.length > 0) {
		...
		logger.info(
			`   - Owner Name: ${firstProperty.displayName || firstProperty.ownerName || "N/A"}`,
		);
		logger.info(
			`   - City: ${firstProperty.situsCity || firstProperty.city || "NOT FOUND"}`,
		);
		logger.info(
			`   - Address: ${firstProperty.streetPrimary || firstProperty.situsAddress || firstProperty.situs || "N/A"}`,
		);
		logger.info(
			`   - Appraised Value: ${firstProperty.appraisedValue || firstProperty.marketValue || firstProperty.mktValue || firstProperty.totalAppraised || "NOT FOUND"}`,
		);
