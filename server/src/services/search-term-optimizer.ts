...
export class SearchTermOptimizer {
	...
	async getStarterTerms(): Promise<string[]> {
		...
		if (analyticsCount === 0) {
			...
			logger.info(
				"🚀 Cold start detected - using optimized 4-character starter terms",
			);
	...
	async updateAnalytics(
		...
		if (existing) {
			...
			const newSuccessfulSearches =
				existing.successfulSearches + (wasSuccessful ? 1 : 0);
			const newFailedSearches =
				existing.failedSearches + (wasSuccessful ? 0 : 1);
	...
	async getOptimizedTerms(config: OptimizerConfig = {}): Promise<string[]> {
		...
		const analytics = await this.prisma.searchTermAnalytics.findMany({
			where: whereClause,
			orderBy: [{ efficiency: "desc" }, { avgResultsPerSearch: "desc" }],
	...
	async getPerformanceStats(): Promise<{
		...
		const totalEfficiency = allAnalytics.reduce(
			(sum, a) => sum + a.efficiency,
			0,
		);
		const totalSuccessRate = allAnalytics.reduce(
			(sum, a) => sum + a.successRate,
			0,
		);
		const totalAvgResults = allAnalytics.reduce(
			(sum, a) => sum + a.avgResultsPerSearch,
			0,
		);
