...
describe('Claude JSON Parsing - Regression Tests', () => {
  ...
  describe('Markdown Code Block Stripping (FIX)', () => {
    ...
    it('should strip ``` wrapper without json tag', async () => {
      ...
      const result = await service.parseNaturalLanguageQuery('Dallas properties');
    ...
    it('should handle response with whitespace around code block', async () => {
      ...
      const result = await service.parseNaturalLanguageQuery('Houston properties');
    ...
    it('should parse clean JSON without code blocks', async () => {
      ...
      const result = await service.parseNaturalLanguageQuery('San Antonio properties');
  ...
  describe('Edge Cases', () => {
    it('should handle JSON with nested objects', async () => {
      ...
      const result = await service.parseNaturalLanguageQuery('Austin expensive');
    ...
    it('should handle JSON with special characters in strings', async () => {
      ...
      const result = await service.parseNaturalLanguageQuery("O'Brien properties");
    ...
    it('should handle unicode characters in response', async () => {
      mockCreate.mockResolvedValue({
        content: [{
          ...
          text: JSON.stringify({
            whereClause: { city: 'Austin' },
            explanation: 'Searching for properties in Austin (valued at > $1M)'
          })
        }],
        usage: { input_tokens: 100, output_tokens: 50 }
      });
  ...
  describe('100% Uptime Requirement', () => {
    it('should NEVER throw an exception to the caller', async () => {
      ...
      const failureScenarios = [
        ...
        () => mockCreate.mockResolvedValue({ content: [{ type: 'text', text: '' }], usage: { input_tokens: 0, output_tokens: 0 } }),
        () => mockCreate.mockResolvedValue({ content: [{ type: 'text', text: 'not json' }], usage: { input_tokens: 0, output_tokens: 0 } }),
        () => mockCreate.mockResolvedValue({ content: [{ type: 'text', text: '{invalid}' }], usage: { input_tokens: 0, output_tokens: 0 } }),
      ];
