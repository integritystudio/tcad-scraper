...
describe("Metrics Service", () => {
	...
	describe("Scrape Job Metrics", () => {
		...
		test("should not increment properties for completed job without count", async () => {
			...
			const propertiesLines = lines.filter(
				(l) =>
					l.includes("tcad_scraper_properties_scraped_total") &&
					!l.startsWith("#"),
			);
	...
	describe("Reset Functionality", () => {
		test("should reset all metrics", async () => {
			...
			const httpRequestLines = lines.filter((l) =>
				l.includes("tcad_scraper_http_requests_total{"),
			);
