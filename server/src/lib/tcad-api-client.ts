...
async function fetchPage(
	...
	if (!res.ok) {
		if (res.status === HTTP_STATUS.UNAUTHORIZED)
			throw new Error(
				`HTTP ${HTTP_STATUS.UNAUTHORIZED} ${HTTP_ERROR.TOKEN_EXPIRED}`,
			);
		if (res.status === HTTP_STATUS.CONFLICT)
			throw new Error(
				`HTTP ${HTTP_STATUS.CONFLICT} ${HTTP_ERROR.RATE_LIMITED}`,
			);
		if (res.status === HTTP_STATUS.GATEWAY_TIMEOUT)
			throw new Error(
				`HTTP ${HTTP_STATUS.GATEWAY_TIMEOUT} ${HTTP_ERROR.GATEWAY_TIMEOUT}`,
			);
		throw new Error(`HTTP ${res.status}`);
	}

	const contentLength =
		res.headers.get("content-length") ?? CONTENT_LENGTH_UNKNOWN;
	...
	if (contentLength !== CONTENT_LENGTH_UNKNOWN) {
		...
		if (!Number.isNaN(declaredBytes) && actualBytes < declaredBytes) {
			throw new Error(
...
export async function fetchTCADProperties(
	...
	for (const pageSize of PAGE_SIZES) {
		...
		try {
			// First page
			const first = await fetchPageWithRetry(
				token,
				searchTerm,
				tcadYear,
				1,
			...
			for (let page = 2; page <= MAX_PAGES; page++) {
				...
				try {
					const next = await fetchPageWithRetry(
						token,
						searchTerm,
						tcadYear,
						page,
					...
					if (msg.includes(HTTP_ERROR.TOKEN_EXPIRED)) {
						throw new Error(
							`${HTTP_ERROR.TOKEN_EXPIRED}: Authorization token expired, needs refresh`,
						);
					}
					if (
						msg.includes(String(HTTP_STATUS.CONFLICT)) ||
						msg.includes(HTTP_ERROR.RATE_LIMITED)
					) {
					...
					if (
						msg.includes(String(HTTP_STATUS.GATEWAY_TIMEOUT)) ||
						msg.includes(HTTP_ERROR.GATEWAY_TIMEOUT)
					) {
 ...
 */
export function mapTCADResultToPropertyData(
