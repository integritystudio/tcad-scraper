...
async function enqueueConstructionBatch() {
    logger_1.logger.info('🏗️  Starting Construction Batch Enqueue');
    ...
    try {
        ...
        for (const term of CONSTRUCTION_TERMS) {
            try {
                ...
                successCount++;
                logger_1.logger.info(`✅ [${successCount}/${CONSTRUCTION_TERMS.length}] Queued: "${term}" (Job ID: ${job.id})`);
            }
            catch (error) {
                failCount++;
        ...
        logger_1.logger.info('✨ Construction batch enqueue completed!');
    }
    catch (error) {
