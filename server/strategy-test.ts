...
async function main() {
	...
	const top2025 = await prisma.$queryRaw<
	...
	console.log(
		`  Base terms: ${top2025
			.filter((t) => t.search_term.length <= 6)
			.map((t) => `${t.search_term}(${t.cnt})`)
			.join(", ")}`,
	);
	...
	console.log(
		`  Dedup ratio: ${s2total > 0 ? ((1 - s2gained / s2total) * 100).toFixed(1) : "N/A"}%`,
	);
	console.log(`  Top 5:`);
	for (const j of s2completed.slice(0, 5)) {
		console.log(
			`    ${(j.searchTerm || "").padEnd(20)} -> ${j.resultCount || 0}`,
		);
	}
	...
	console.log(
		`  Dedup ratio: ${s3total > 0 ? ((1 - s3gained / s3total) * 100).toFixed(1) : "N/A"}%`,
	);
	console.log(`  Top 5:`);
	...
	console.log(
		`  Terms:            ${String(s2terms.length).padStart(5)}            ${String(s3terms.length).padStart(5)}`,
	);
	console.log(
		`  Results fetched:  ${String(s2total).padStart(5)}            ${String(s3total).padStart(5)}`,
	);
	console.log(
		`  New unique:       ${String(s2gained).padStart(5)}            ${String(s3gained).padStart(5)}`,
	);
	console.log(
		`  Yield/term:       ${(s2gained / (s2terms.length || 1)).toFixed(1).padStart(5)}            ${(s3gained / (s3terms.length || 1)).toFixed(1).padStart(5)}`,
	);
	console.log(
		`  Dedup %:          ${(s2total > 0 ? ((1 - s2gained / s2total) * 100).toFixed(1) : "N/A").padStart(5)}            ${(s3total > 0 ? ((1 - s3gained / s3total) * 100).toFixed(1) : "N/A").padStart(5)}`,
	);
	console.log(
		`  Elapsed:          ${(`${s2elapsed}s`).padStart(5)}            ${(`${s3elapsed}s`).padStart(5)}`,
	);
	console.log(`\n  Final 2025 count: ${finalCount.toLocaleString()}`);
	console.log(`  Gap to 420K: ${(420000 - finalCount).toLocaleString()}`);
	console.log(
		`  Winner: ${s2gained > s3gained ? "Strategy 2 (static names)" : s3gained > s2gained ? "Strategy 3 (prefix expansion)" : "TIE"}`,
	);
}
