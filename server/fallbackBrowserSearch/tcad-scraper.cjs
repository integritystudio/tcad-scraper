...
const CONFIG = {
	...
	CONCURRENT_WORKERS: parseInt(process.env.CONCURRENT_WORKERS, 10),
...
worker.on("progress", (job, progress) => {
	console.log(
		`📊 Job ${job.id} progress: ${progress.current}/${progress.total} - ${progress.currentTerm}`,
	);
});
...
async function addBatch(searchTerms, options = {}) {
	...
	for (let i = 0; i < batches.length; i++) {
		...
		jobs.push(job);
		console.log(
			`   ✓ Added batch ${i + 1}/${batches.length} (Job ID: ${job.id})`,
		);
	}
...
async function processSearchTermsFromFile(filePath) {
	...
	try {
		...
		console.log(
			`📄 Loaded ${searchTerms.length} search terms from ${filePath}`,
		);
