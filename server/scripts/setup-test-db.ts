...
async function main() {
	...
	try {
		...
		if (!postgresRunning) {
			...
			console.log(
				`  - Docker: docker run --name postgres -e POSTGRES_PASSWORD=${config.dbPassword} -p 5432:5432 -d postgres:16`,
			);
		...
		console.log("");
		console.log(
			`Database URL: postgresql://${config.dbUser}:${config.dbPassword}@${config.dbHost}:${config.dbPort}/${config.dbName}`,
		);
		console.log("");
